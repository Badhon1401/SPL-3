...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public PerformanceAnalyticsResponse getAnalytics(Long userId) {
        ...
        long total    = all.size();
        long accepted = all.stream().filter(s -> s.getVerdict() == Submission.Verdict.ACCEPTED).count();
        long unique   = submissionRepository.countDistinctAcceptedProblemsByUserId(userId);
        double rate   = total == 0 ? 0.0 : Math.round((double) accepted / total * 10000.0) / 100.0;
        ...
        // Topic breakdown — combined, accepted only
        Map<String, Long> topicBreakdown = new LinkedHashMap<>();
        all.stream().filter(s -> s.getVerdict() == Submission.Verdict.ACCEPTED)
        ...
        // Difficulty breakdown — normalised across platforms
        Map<String, Long> difficultyBreakdown = all.stream()
                .filter(s -> s.getVerdict() == Submission.Verdict.ACCEPTED)
                .collect(Collectors.groupingBy(s -> normaliseDifficulty(s.getProblem()), Collectors.counting()));

        DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        Map<String, Long> activityHeatmap = all.stream()
                .filter(s -> s.getSubmittedAt() != null
                        && s.getSubmittedAt().isAfter(LocalDateTime.now().minusDays(365)))
                .collect(Collectors.groupingBy(s -> s.getSubmittedAt().format(fmt), Collectors.counting()));

        Map<String, Long> verdictDistribution = all.stream()
    ...
    private long[] calculateStreaks(List<Submission> submissions) {
        ...
        for (int i = 1; i < sorted.size(); i++) {
            if (sorted.get(i).equals(sorted.get(i - 1).plusDays(1))) { temp++; longest = Math.max(longest, temp); }
            else temp = 1;
        }
        long current = 0;
    ...
    private Map<String, Double> computeWeaknessScores(List<Submission> subs) {
        Map<String, Long> total = new HashMap<>(), failed = new HashMap<>();
    ...
    private String normaliseDifficulty(com.codepulse.entity.Problem p) {
        if (p.getDifficultyLabel() != null)
            return switch (p.getDifficultyLabel()) {
                case "Easy" -> "Easy"; case "Medium" -> "Medium"; case "Hard" -> "Hard"; default -> "Medium";
            };
    ...
    private Map<String, PlatformStats> buildPlatformBreakdown(List<Submission> all) {
        ...
        byPlatform.forEach((platform, subs) -> {
            ...
            long uniq = subs.stream().filter(s -> s.getVerdict() == Submission.Verdict.ACCEPTED)
            ...
            subs.stream().filter(s -> s.getVerdict() == Submission.Verdict.ACCEPTED)
                    .forEach(s -> s.getProblem().getTopics().forEach(t -> topics.merge(t.getName(), 1L, Long::sum)));
            Map<String, Long> verdicts = subs.stream().collect(Collectors.groupingBy(
                    s -> VERDICT_LABEL.getOrDefault(s.getVerdict().name(), s.getVerdict().name()), Collectors.counting()));
            result.put(platform, PlatformStats.builder()
                    .totalSubmissions(tot).acceptedCount(ac).uniqueSolved(uniq)
                    .acceptanceRate(r).topicBreakdown(topics).verdictDistribution(verdicts).build());
        });
        return result;
    }
    ...
    private List<RatingPoint> fetchCfRatingTrend(User user) {
        try {
            ...
            CfRatingResponse resp = webClientBuilder.build().get()
                    .uri(cfBaseUrl + "/user.rating?handle=" + user.getCodeforcesHandle())
                    .retrieve().bodyToMono(CfRatingResponse.class).block();
            if (resp == null || !"OK".equals(resp.getStatus())) return List.of();
            DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            return resp.getResult().stream().map(e -> RatingPoint.builder()
                    .date(LocalDateTime.ofEpochSecond(((Number)e.get("ratingUpdateTimeSeconds")).longValue(),
                            0, java.time.ZoneOffset.UTC).format(fmt))
                    .rating(((Number)e.get("newRating")).intValue()).build()).toList();
        } catch (Exception e) { log.warn("CF rating trend: {}", e.getMessage()); return List.of(); }
    }

    @Data @JsonIgnoreProperties(ignoreUnknown = true)
    static class CfRatingResponse { private String status; private List<Map<String,Object>> result; }
}